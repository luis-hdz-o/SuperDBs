...
fecha1= '2011-01-01' ##valores a definir por el usuario
fecha2= '2011-01-15' #IMPORTANTE!!!! Las semanas van de la 0 a la 51 en SQL

query="SELECT DATE(fechacompra) as FECHA, SUM(cantidad*precioventa) as DINERO FROM sales WHERE fechacompra BETWEEN '2011-01-01' AND '2011-01-15' GROUP BY fechacompra;".format(fecha1, fecha2) #query de sql obtiene una tabla con factura y el conteo de los SKU (Ventas por país)
...
'''
plot=df.plot.bar(x="FECHA",y='DINERO',rot=0, figsize= (20,5))
fig = plot.get_figure()
fig.savefig("barventasmes.png")


plot=df.plot.scatter(x="FECHA",y="DINERO",rot=0, figsize= (20,5))
fig = plot.get_figure()
